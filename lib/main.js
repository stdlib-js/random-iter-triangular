...
function iterator( a, b, c, options ) {
	...
	if ( !isNumber( a ) || isnan( a ) ) {
		throw new TypeError( format( '0qV6v', a ) );
	}
	if ( !isNumber( b ) || isnan( b ) ) {
		throw new TypeError( format( '0qV6w', b ) );
	}
	if ( !isNumber( c ) || isnan( c ) ) {
		throw new TypeError( format( '0qV7C', c ) );
	}
	if ( !(a <= c && c <= b) ) {
		throw new RangeError( format( '0qV9C', 'a <= c <= b', a, b, c ) );
	}
	if ( arguments.length > 3 ) {
		...
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0qV2t', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
