...
function iterator( a, b, c, options ) {
	...
	if ( !isNumber( a ) || isnan( a ) ) {
		throw new TypeError( format( '0qV6v,NI', a ) );
	}
	if ( !isNumber( b ) || isnan( b ) ) {
		throw new TypeError( format( '0qV6w,NJ', b ) );
	}
	if ( !isNumber( c ) || isnan( c ) ) {
		throw new TypeError( format( '0qV7C,NR', c ) );
	}
	if ( !(a <= c && c <= b) ) {
		throw new RangeError( format( '0qV9C,HP', 'a <= c <= b', a, b, c ) );
	}
	if ( arguments.length > 3 ) {
		...
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0qV2t,G9', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
